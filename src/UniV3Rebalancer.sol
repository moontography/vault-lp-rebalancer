...
contract UniV3Rebalancer is AutomationCompatibleInterface, ERC4626 {
    ...
    function _getPendingFees() internal view returns (uint256 _fee0, uint256 _fee1) {
        ...
        if (_currentTick >= _currentLowerTick) {
            ...
            _feeGrowthBelow0X128 = _subIn256(_global0X128, _feeGrowthOutside0LowerX128);
            _feeGrowthBelow1X128 = _subIn256(_global1X128, _feeGrowthOutside1LowerX128);
        }
        ...
        if (_currentTick < _currentUpperTick) {
            ...
            _feeGrowthAbove0X128 = _subIn256(_global0X128, _feeGrowthOutside0UpperX128);
            _feeGrowthAbove1X128 = _subIn256(_global1X128, _feeGrowthOutside1UpperX128);
        }

        _fee0 =
            (uint256(_liquidity) *
                _subIn256(
                    _subIn256(_subIn256(_global0X128, _feeGrowthBelow0X128), _feeGrowthAbove0X128),
        ...
        _fee1 =
            (uint256(_liquidity) *
                _subIn256(
                    _subIn256(_subIn256(_global1X128, _feeGrowthBelow1X128), _feeGrowthAbove1X128),
    ...
    function _subIn256(uint256 _x, uint256 _y) private pure returns (uint256) {
        unchecked {
            if (_x < _y) {
                // Simulate wrap-around behavior (underflow) by leveraging uint256 wrap-around feature.
                return (type(uint256).max - (_y - _x)) + 1;
            } else {
                return _x - _y;
            }
        }
